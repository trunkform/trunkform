...
Then ensure unit testing coverage is still at 100%. Check the .tools array to determine the testing framework and coverage tools. If testing is not 100% line coverage, immediately update .trunkform file with 
...
line-coverage goals. Check the .tools array to determine the testing framework and coverage tools. ` + CriticalPrefix + ` and tested for line coverage. ` + NeverGuess, map[string]any{}
}
...
func (tt TrunkformTool) handleTool(ctx context.Context, req mcp.CallToolRequest, args formatArgs) (*mcp.CallToolResult, error) {
  if args.Version == "" && len(args.Tools) == 0 && args.Steering == "" && len(args.Related) == 0 && len(args.Rubric) == 0 {
    return mcp.NewToolResultText(InstructionPrefix + ToolDescription + NeverGuess), nil
  }
...
func (tt TrunkformTool) Register(s *server.MCPServer) {
  ...
  t := mcp.NewTool(
    "trunkform",
    mcp.WithDescription(InstructionPrefix + ToolDescription),
